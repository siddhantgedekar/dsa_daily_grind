...
### Generics
- **Problem Explanation**: Demonstrates a generic method `printArray` that works with arrays of any reference type (`String[]`, `Integer[]`), printing each element.
```
...
### Comparators
- **Problem Explanation**: Sort a list of `Player` objects by descending score and then lexicographically by name when scores are tied. Updated implementation reads from STDIN without interactive prompts and documents the comparator behavior.
```
import java.util.*;
...
### Comparator2
- **Problem Explanation**: Sort a list of `Student` objects by CGPA descending, then name ascending, then id descending. The program reads student records from STDIN and prints the sorted result.
```
import java.util.*;
